...
async def show_search(message: types.Message, state: FSMContext):
    ...
    if len(get_categories) >= 1:
        ...
        await message.answer("<b>💙 Выберите необходимый товар:</b>", reply_markup=get_kb)
    else:
        await message.answer("<b>💙 Товара в данный момент не имеется.</b>")
...
# Обработка кнопки "Профиль"
@dp.message_handler(text="💙 | Discord", state="*")
async def show_profile(message: types.Message, state: FSMContext):
    await state.finish()
    await message.answer("<b>💙 Discord сервер: discord.gg/diamondshop</b>")
...
async def open_category_for_buy_item(call: CallbackQuery, state: FSMContext):
    ...
    if len(get_positions) >= 1:
        await call.message.edit_text("<b>💙 Выберите необходимый товар:</b>",
                                     reply_markup=get_kb)
    else:
...
async def buy_item_next_page_position(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.delete()
    await call.message.answer("<b>💙 Выберите необходимый товар:</b>",
