...
async def get_id_for_change_contact(message: types.Message, state: FSMContext):
    ...
    if msg.isdigit():
        ...
        if get_status_user is None:
            ...
            await state.finish()
            msg = f"📕 <b>По заменам писать сюда ➡ <a href='tg://user?id={msg}'>Саппорт</a>\n\n💙 Для получения замены невалида присылайте сразу видеозапись подтверждающую невалидность гифта, а так же сообщение о покупке из бота :)</b>"
